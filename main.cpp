...
class HelloTriangleApplication
{
  ...
  private:
	...
	void pickPhysicalDevice()
	{
		...
		int selectedId;
		if (!(std::cin >> selectedId) || selectedId < 0 || selectedId >= deviceCount)
		{
			throw std::runtime_error("Invalid device selection!");
		}

		physicalDevice = deviceMap[selectedId].second;
		std::cout << "Selected device: " << deviceMap[selectedId].first << std::endl;
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) const
	{
		...
		for (QueueFamilyIndex i = 0; i < queueFamilies.size(); i++)
		{
